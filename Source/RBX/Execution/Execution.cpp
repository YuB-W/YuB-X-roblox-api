...
inline uintptr_t MaxCaps = 0xEFFFFFFFFFFFFFFF;
...
void CExecution::Send(lua_State* L, std::string Source, bool Compile, int Yield) {
    ...
    RBX::SetThreadCapabilities((uintptr_t)Thread, MaxCaps);
    ...
    if (LoadResult == LUA_OK) {
        RBX::SetProto(((Closure*)lua_topointer(Thread, -1))->l.p, &MaxCaps);

        RBX::Task__Defer(Thread);
        //RBX::Print(0, "YuB-X LUA_OK: " + LoadResult);
    }
    else {
        //RBX::Print(3, "YuB-X Error: " + LoadResult);
