...
lua_State* lua_newthread(lua_State* L)
{
    ...
    if (L1 == nullptr || L1->tt != LUA_TTHREAD) 
        return nullptr;
...
void lua_clonefunction(lua_State* L, int idx)
{
    ...
    if (cl->isC) {
        ...
        lua_Continuation cont = cl->c.cont;
        const char* debugname = cl->c.debugname;

        newcl->c.f = func;
        newcl->c.cont = cont;
        newcl->c.debugname = (debugname == NULL ? "" : debugname);

        setclvalue(L, L->top, newcl);
        api_incr_top(L);
    }
    else {
        Closure* newcl = luaF_newLclosure(L, cl->nupvalues, cl->env, cl->l.p);
